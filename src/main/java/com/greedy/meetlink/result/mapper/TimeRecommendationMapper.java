 ...
 *
 * [분리 이유]
 * 기존에 TimeRecommendationResponse 내부 정적 메서드로 존재하던 변환 로직을 이곳으로 이동.
 * - DTO는 데이터 구조 정의만 담당 (SRP 준수)
 * - 변환 로직의 테스트 및 유지보수 용이성 향상
 */
public class TimeRecommendationMapper {
    ...
    /**
     * TimeAvailability 리스트 → 히트맵 응답 변환
     * - SPECIFIC_DATE 모드: 날짜 기준 그룹핑
     * - WEEKLY 모드: 요일 기준 그룹핑
     */
    private static List<HeatmapResponse> toHeatmapResponses(List<TimeAvailability> availabilities) {
        ...
        Map<Object, List<TimeAvailability>> grouped = availabilities.stream()
                .collect(Collectors.groupingBy(ta ->
                        ta.getDate() != null ? ta.getDate() : ta.getDayOfWeek()
                ));
    ...
    private static HeatmapResponse toHeatmapResponse(Object key, List<TimeAvailability> groupedAvailabilities) {
    ...
    private static List<TimeSlotResponse> toTimeSlotResponses(List<TimeAvailability> availabilities) {
        Map<LocalTime, Long> timeSlotCounts = availabilities.stream()
        ...
        return timeSlotCounts.entrySet().stream()
                .map(entry -> TimeSlotResponse.builder()
     ...
     */
    private static List<TimeCandidateResponse> toCandidateResponses(List<TimeCandidate> candidates) {
