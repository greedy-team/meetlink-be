 ...
 *
 * API: GET https://apis.openapi.sk.com/tmap/pois
 * 문서: https://openapi.sk.com/products/detail?svcSeq=36
 *
 * 점수 상위 K개 후보 좌표 주변에서 실제 장소를 검색하는 데 사용
 * 카테고리: 카페(CE7), 음식점(FD6), 편의시설(AD5) 복합 검색
 */
@Slf4j
@Component
public class TMapPoiClient {
    ...
    public List<PoiPlace> searchNearby(Coordinate center, int radiusMeters) {
        try {
            PoiSearchResponse response = webClient.get()
                    .uri(uriBuilder -> uriBuilder
                            ...
                            .queryParam("radius", radiusMeters / 1000.0) // API 단위: km
            ...
            log.error("TMap POI API 오류: status={}, body={}", e.getStatusCode(), e.getResponseBodyAsString());
            return Collections.emptyList();
        } catch (Exception e) {
