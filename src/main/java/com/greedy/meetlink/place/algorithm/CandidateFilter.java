...
public class CandidateFilter {
     ...
     * application.yml 예시:
     *   tmap:
     *     call-delay-ms: 1000   # 무료 플랜
     *     # call-delay-ms: 0   # 유료 플랜
     *
     * FakeTransitClient 사용 시 0으로 설정하면 테스트 속도 향상.
     */
     ...
     *
     * [전략] API 호출 최소화를 위해 2단계로 진행
     *   1단계: 기하중심에서 가장 먼 참여자(샘플) 먼저 검증 → 60분 초과 시 즉시 탈락
     *   2단계: 나머지 참여자 검증 → API 실패 or 60분 초과 시 탈락
     *
     * ✅ 리팩토링: 기존 2단계 루프에서 나머지 참여자는 null 체크만 했으나,
     *    MAX_TRAVEL_TIME_MINUTES 초과도 탈락 조건에 포함되도록 수정.
     ...
     */
    private Set<Coordinate> selectSampleParticipants(List<Coordinate> participants, Coordinate center) {
     ...
     *
     * ✅ 리팩토링: null 체크 외에 MAX_TRAVEL_TIME_MINUTES 초과도 탈락 조건 추가
     */
    private boolean isFailedOnRemainder(
        ...
        for (Coordinate p : participants) {
            ...
            Double travelTime = callWithDelay(p, candidate);
            if (isInvalidTravelTime(travelTime)) {
                log.warn("나머지 참여자 필터 탈락: candidate={}, participant={}, time={}",
                        candidate, p, travelTime);
                return true;
            }
            cache.put(p, travelTime);
    ...
    /**
     * TMap API 호출 + 딜레이
     * ✅ 리팩토링: 딜레이값을 @Value로 주입받아 환경별 조정 가능
     */
