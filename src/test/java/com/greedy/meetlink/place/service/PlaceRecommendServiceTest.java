...
class PlaceRecommendServiceTest {
    ...
    void setUp() {
        ...
        placeRecommendService = new PlaceRecommendService(
    ...
    void recommendAndSave_Success() {
        ...
        given(poiClient.searchNearby(any())).willReturn(List.of(
                new PoiPlace("테스트 카페", "서울시 어딘가", 37.5, 127.0)
        ));

        // Mock Repositories
        given(placeCandidateRepository.save(any())).willAnswer(invocation -> invocation.getArgument(0));
        given(meetingResultRepository.findByMeeting(meeting)).willReturn(Optional.empty());
        given(meetingResultRepository.save(any())).willAnswer(invocation -> invocation.getArgument(0));
    ...
    private Participant createParticipant(Long id, Meeting meeting, String nick, double lat, double lon) {
