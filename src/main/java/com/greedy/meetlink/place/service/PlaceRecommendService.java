 ...
 * 장소 추천 서비스 (오케스트레이터)
 *
 * <p>✅ 트랜잭션 전략: - runAlgorithm() : @Transactional 없음 → TMap API 호출(최대 수분)이 포함되어 DB 커넥션을 장시간 점유하지
 * 않도록 분리 - saveResults() : @Transactional → 모든 DB 저장을 하나의 트랜잭션으로 묶어 알고리즘 성공 후 부분 저장 방지 -
 * recommendAndSave() : 두 메서드를 순서대로 호출하는 public 진입점
 *
 * <p>✅ PlaceCandidate ↔ MeetingResult 연결 전략: - PlaceCandidate는 TOP_K(3)개 전부 저장 → 사용자에게 후보 선택지 제공 -
...
public class PlaceRecommendService {
    ...
    /** TMap API 호출을 포함한 알고리즘 전체 실행 @Transactional을 걸지 않아 DB 커넥션을 API 대기 시간 동안 점유하지 않음 */
     ...
     *
     * <p>저장 순서: 1. PlaceCandidate (rank 1~3) 2. PlaceTravelInfo (참여자 × 후보 수) 3.
     * MeetingResult.placeCandidate = rank 1 후보
     */
    @Transactional
    ...
    /** PlaceTravelInfo 저장 (참여자별 이동시간) saved[i] ↔ matchedPlaces[i] ↔ participants[j] 인덱스 매핑 */
    ...
    /** MeetingResult에 rank=1 PlaceCandidate 연결 MeetingResult가 없으면 새로 생성 (findOrCreate 패턴) */
