 ...
 * 추천 장소 조회 전용 서비스
 *
 * PlaceCandidateController.getCandidates()의 조회 로직을
 * 컨트롤러에서 분리하여 서비스 레이어로 이동.
 *
 * [N+1 방지 전략]
 *   1. PlaceCandidate 목록 1번 조회
 *   2. PlaceTravelInfo 전체를 IN 쿼리로 1번 조회 (fetch join으로 Participant 포함)
 *   3. candidateId 기준으로 groupingBy → 메모리에서 매핑
 *   총 2번의 쿼리로 처리
 */
@Service
...
public class PlaceCandidateQueryService {
    ...
    public PlaceCandidateListResponse getCandidates(Meeting meeting) {
        ...
        List<PlaceCandidate> candidates = placeCandidateRepository
                .findByMeetingOrderByRankAsc(meeting);
        ...
        List<PlaceTravelInfo> allTravelInfos = placeTravelInfoRepository
                .findByPlaceCandidatesWithParticipant(candidates);

        // 3. candidateId 기준으로 그룹핑
        Map<Long, List<PlaceTravelInfo>> travelInfoMap = allTravelInfos.stream()
                .collect(Collectors.groupingBy(
                        pti -> pti.getPlaceCandidate().getId()
                ));

        // 4. 응답 조립
        List<RecommendedPlaceResponse> recommendedPlaces = candidates.stream()
                .map(candidate -> RecommendedPlaceResponse.of(
                        candidate,
                        travelInfoMap.getOrDefault(candidate.getId(), List.of())
                ))
                .toList();
