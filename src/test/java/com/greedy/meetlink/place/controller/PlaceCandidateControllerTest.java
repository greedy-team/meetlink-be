...
class PlaceCandidateControllerTest {
    ...
    @MockitoBean
    PlaceCandidateQueryService placeCandidateQueryService;
    ...
    class Calculate {
        ...
        void calculate_success() throws Exception {
            // given
            given(meetingRepository.findByCode("ABC123"))
                    .willReturn(Optional.of(fakeMeeting()));
            willDoNothing().given(placeRecommendService).recommendAndSave(any());

            // when & then
            mockMvc.perform(post("/meetings/ABC123/candidates/place")
        ...
        void calculate_meetingNotFound() throws Exception {
            // given
            given(meetingRepository.findByCode("INVALID"))
                    .willReturn(Optional.empty());

            // when & then
            mockMvc.perform(post("/meetings/INVALID/candidates/place")
                            .contentType(MediaType.APPLICATION_JSON))
                    .andDo(print())
        ...
        void calculate_notEnoughParticipants() throws Exception {
            ...
            willThrow(new IllegalArgumentException("장소 추천을 위해 참여자가 2명 이상 필요합니다."))
                    .given(placeRecommendService).recommendAndSave(any());
    ...
    class GetCandidates {
        ...
        void getCandidates_success() throws Exception {
            ...
            // when & then
            mockMvc.perform(get("/meetings/ABC123/candidates/place")
                            .contentType(MediaType.APPLICATION_JSON))
                    .andDo(print())
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.status").value(true))
                    ...
                    // rank=1 장소 검증
                    .andExpect(jsonPath("$.result.recommendedPlaces[0].placeName").value("스타벅스 강남점"))
                    .andExpect(jsonPath("$.result.recommendedPlaces[0].roadAddress").value("서울 강남구 테헤란로 101"))
                    ...
                    .andExpect(jsonPath("$.result.recommendedPlaces[0].averageDuration").value(1530))
                    .andExpect(jsonPath("$.result.recommendedPlaces[0].maxDuration").value(2400))
                    // participantDurations 검증
                    .andExpect(jsonPath("$.result.recommendedPlaces[0].participantDurations").isArray())
                    .andExpect(jsonPath("$.result.recommendedPlaces[0].participantDurations.length()").value(2))
                    .andExpect(jsonPath("$.result.recommendedPlaces[0].participantDurations[0].nickname").value("홍길동"))
                    .andExpect(jsonPath("$.result.recommendedPlaces[0].participantDurations[0].duration").value(1500))
                    // rank=2 장소 검증
        ...
        void getCandidates_meetingNotFound() throws Exception {
            ...
            // when & then
            mockMvc.perform(get("/meetings/INVALID/candidates/place")
                            .contentType(MediaType.APPLICATION_JSON))
                    .andDo(print())
                    .andExpect(status().isNotFound());
        }
        ...
        void getCandidates_pathDataNull() throws Exception {
            ...
            mockMvc.perform(get("/meetings/ABC123/candidates/place")
                    ...
                    .andDo(print())
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.result.recommendedPlaces[0].participantDurations[0].pathData")
                            .doesNotExist());
        }
    }
    ...
    private PlaceCandidateListResponse fakePlaceCandidateListResponseWithNullPath() {
        List<ParticipantDurationResponse> participants = List.of(
                new ParticipantDurationResponse("홍길동", 1500, null)
        );
        List<RecommendedPlaceResponse> places = List.of(
                new RecommendedPlaceResponse(
                        "스타벅스 강남점", "서울 강남구 테헤란로 101",
                        37.4979, 127.0276,
                        1, 1530, 2400, participants)
        );
        return PlaceCandidateListResponse.of(places);
    }
}
